...
queue='batch' # Does not have a flag. Change if necessary
port=5959 # Port at which JupyterLab will run: -p flag
walltime=30:00 # How long the job will run: -t flag
account='stf011' # Account you belong to: -A flag
verbose=1 # Set to 1 if you want more print statements: -v flag
...
if [[ $verbose -eq 1 ]]; then
  echo "sending: sbatch -A ${account} -p ${queue} -N ${nnodes} -n ${nnodes} -c 32 -J Jupyter --mem=128G -t ${walltime}  start_jlab.sh ${port} ${HOSTNAME} ${verbose}"
fi
...
sbatch -A ${account} -p ${queue} -N ${nnodes} -n ${nnodes} -c 32 -J Jupyter --mem=120G -t ${walltime}  -o ./%x-%j.o -e ./%x-%j.e start_jlab.sh ${port} ${HOSTNAME} ${verbose}